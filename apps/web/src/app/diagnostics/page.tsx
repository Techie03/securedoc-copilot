...
export default function DiagnosticsPage() {
  ...
  return (
    <div className="relative min-h-[calc(100vh-4rem)] bg-slate-950 text-slate-100 py-12 px-4 sm:px-6 lg:px-8 overflow-hidden">
      ...
      <div className="max-w-4xl mx-auto space-y-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          ...
          <div className="md:col-span-1 rounded-2xl border border-white/5 bg-slate-900/60 p-6 backdrop-blur-xl space-y-6">
            ...
            <div className="space-y-4 text-sm">
              <div>
                ...
                <div className="font-mono text-xs bg-slate-950 p-2.5 rounded-lg border border-white/5 break-all text-slate-300">
                  {clientOrigin}
                </div>
              </div>
              <div>
          ...
          <div className="md:col-span-2 space-y-6">
            ...
            <div className="rounded-2xl border border-white/5 bg-slate-900/60 p-6 backdrop-blur-xl">
              ...
              <div className="space-y-4">
                <div className="font-mono text-xs bg-slate-950 p-4 rounded-xl border border-white/5 whitespace-pre-wrap break-all text-slate-300 max-h-40 overflow-y-auto">
                  {backendHealth.detail}
                </div>
                {backendHealth.data && (
                  <div className="text-xs bg-slate-950/40 p-4 rounded-xl border border-white/5">
                    <div className="text-slate-400 font-semibold mb-2">Health Report Payload:</div>
                    <pre className="font-mono text-slate-300">{JSON.stringify(backendHealth.data, null, 2)}</pre>
                  </div>
                )}
              </div>
            </div>
                ...
                {backendDiagnostics.status === 'Connected' && backendDiagnostics.data ? (
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div className="p-4 bg-slate-950 rounded-xl border border-white/5 space-y-1">
                      <div className="text-xs text-slate-400 font-semibold uppercase">PostgreSQL Database</div>
                      <div className={`text-sm font-semibold ${backendDiagnostics.data.database === 'connected' ? 'text-emerald-400' : 'text-rose-400'}`}>
                        {backendDiagnostics.data.database}
                      </div>
                    </div>
                    <div className="p-4 bg-slate-950 rounded-xl border border-white/5 space-y-1">
                      <div className="text-xs text-slate-400 font-semibold uppercase">Qdrant Vector Database</div>
                      <div className={`text-sm font-semibold ${backendDiagnostics.data.qdrant === 'connected' ? 'text-emerald-400' : 'text-rose-400'}`}>
                        {backendDiagnostics.data.qdrant}
                      </div>
                    </div>
                    <div className="p-4 bg-slate-950 rounded-xl border border-white/5 space-y-1">
                      <div className="text-xs text-slate-400 font-semibold uppercase">Redis Cache/Queue</div>
                      <div className={`text-sm font-semibold ${backendDiagnostics.data.redis === 'connected' ? 'text-emerald-400' : 'text-rose-400'}`}>
                        {backendDiagnostics.data.redis}
                      </div>
                    </div>
                    <div className="p-4 bg-slate-950 rounded-xl border border-white/5 space-y-1">
                      <div className="text-xs text-slate-400 font-semibold uppercase">NVIDIA NIM AI</div>
                      <div className={`text-sm font-semibold ${backendDiagnostics.data.nvidia_nim === 'connected' ? 'text-emerald-400' : 'text-rose-400'}`}>
                        {backendDiagnostics.data.nvidia_nim}
                      </div>
                    </div>
                    <div className="sm:col-span-2 p-4 bg-slate-950 rounded-xl border border-white/5 space-y-2">
                      <div className="text-xs text-slate-400 font-semibold uppercase">Backend OAuth Setup</div>
                      <div className="grid grid-cols-2 gap-2 text-xs">
                        <div className="flex items-center gap-2">
                          <Key className="h-3.5 w-3.5 text-slate-500" />
                          <span>GitHub: {backendDiagnostics.data.oauth?.github_configured ? '✅ Ready' : '❌ Not Configured'}</span>
                        </div>
                        <div className="flex items-center gap-2">
                          <Key className="h-3.5 w-3.5 text-slate-500" />
                          <span>Google: {backendDiagnostics.data.oauth?.google_configured ? '✅ Ready' : '❌ Not Configured'}</span>
                        </div>
                      </div>
                    </div>
                  </div>
                ) : (
                  <div className="font-mono text-xs bg-slate-950 p-4 rounded-xl border border-white/5 whitespace-pre-wrap break-all text-slate-300">
        ...
        <div className="rounded-2xl border border-yellow-500/20 bg-yellow-500/5 p-5 flex items-start gap-4">
          ...
          <div className="space-y-1 text-sm text-yellow-200/80">
            <h4 className="font-semibold text-yellow-400">Troubleshooting Network / Fetch Failures:</h4>
            <ul className="list-disc pl-4 space-y-1 text-xs">
              <li>Check your browser's Developer Tools Console (F12) for detailed errors.</li>
              <li>If you see a CORS error, check the Hugging Face Space settings environment variable <code className="bg-slate-900 px-1 py-0.5 rounded text-white">ALLOWED_ORIGINS</code>. It must match your Vercel deployment URL (e.g. <code className="bg-slate-900 px-1 py-0.5 rounded text-white">https://xxx.vercel.app</code>) or be set to <code className="bg-slate-900 px-1 py-0.5 rounded text-white">*</code>.</li>
              <li>Verify that the Vercel frontend build configuration has the environment variable <code className="bg-slate-900 px-1 py-0.5 rounded text-white">NEXT_PUBLIC_API_URL</code> set to the exact HTTPS URL of the Hugging Face Space (e.g. <code className="bg-slate-900 px-1 py-0.5 rounded text-white">https://username-spacename.hf.space</code>). Do not use the embedding URL wrapping the iframe.</li>
            </ul>
          </div>
        </div>
        ...
        <div className="text-center">
          <Link href="/login" className="text-xs text-cyan-400 hover:text-cyan-300 font-medium flex items-center justify-center gap-1">
