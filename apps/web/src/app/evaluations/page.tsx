...
const ROUTE_COLORS: Record<string, string> = {
  rag: 'text-violet-400 bg-violet-400/10 border-violet-500/20',
  coding: 'text-amber-400 bg-amber-400/10 border-amber-500/20',
  summary: 'text-emerald-400 bg-emerald-400/10 border-emerald-500/20',
  compare: 'text-pink-400 bg-pink-400/10 border-pink-500/20',
  memory: 'text-cyan-400 bg-cyan-400/10 border-cyan-500/20',
...
export default function EvaluationsPage() {
  ...
  return (
    <div className="flex-1 bg-slate-950 text-slate-100 min-h-[calc(100vh-4rem)] py-8 px-4 sm:px-6 lg:px-8 relative">
      ...
      <div className="max-w-7xl mx-auto space-y-8">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 border-b border-white/10 pb-6">
          <div>
            <h1 className="text-2xl font-bold tracking-tight text-white flex items-center gap-2.5">
              <LineChart className="h-6 w-6 text-cyan-400 animate-pulse" />
          ...
          <button
            onClick={loadLogs}
            disabled={loading}
            className="flex items-center gap-2 rounded-xl bg-white/5 border border-white/5 px-4 py-2 text-xs font-semibold text-slate-200 hover:bg-white/10 transition-all cursor-pointer disabled:opacity-50"
          >
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5">
          {/* Card 1: Latency */}
          <div className="rounded-2xl border border-white/5 bg-slate-900/30 p-5 backdrop-blur-md relative overflow-hidden group">
            ...
            <div className="mt-3 text-3xl font-extrabold text-white">{avgLatency} ms</div>
          ...
          <div className="rounded-2xl border border-white/5 bg-slate-900/30 p-5 backdrop-blur-md relative overflow-hidden group">
            ...
            <div className="mt-3 text-3xl font-extrabold text-emerald-400">{avgFaithfulness}%</div>
          ...
          <div className="rounded-2xl border border-white/5 bg-slate-900/30 p-5 backdrop-blur-md relative overflow-hidden group">
            ...
            <div className="mt-3 text-3xl font-extrabold text-violet-400">{avgRelevance}%</div>
            <div className="mt-1.5 text-[10px] text-slate-500">
          ...
          <div className="rounded-2xl border border-white/5 bg-slate-900/30 p-5 backdrop-blur-md relative overflow-hidden group">
            ...
            <div className="mt-3 text-3xl font-extrabold text-pink-400">${totalCost.toFixed(5)}</div>
            <div className="mt-1.5 text-[10px] text-slate-500">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="lg:col-span-1 space-y-6">
            ...
            <div className="rounded-3xl border border-white/5 bg-slate-900/20 p-6 backdrop-blur-xl space-y-4">
              ...
              <div className="space-y-3.5 pt-2">
                  ...
                  return (
                    <div key={route} className="text-xs">
                      <div className="flex items-center justify-between text-[11px] font-semibold mb-1">
                        ...
                        <span className="text-slate-300 font-bold">{count} runs ({pct}%)</span>
                      </div>
                ...
                {totalCount === 0 && (
                  <div className="text-xs text-slate-500 text-center py-4">No route breakdowns recorded.</div>
                )}
              </div>
            </div>
            ...
            <div className="rounded-3xl border border-white/5 bg-slate-900/20 p-6 backdrop-blur-xl space-y-4">
              ...
              <div className="space-y-3 text-xs leading-relaxed text-slate-400">
                <div>
                  <h4 className="font-bold text-slate-200 mb-0.5">Answer Faithfulness</h4>
                ...
                <div>
                  <h4 className="font-bold text-slate-200 mb-0.5">Context Relevance</h4>
                ...
                <div>
                  <h4 className="font-bold text-slate-200 mb-0.5">Hallucination Risk</h4>
          ...
          <div className="lg:col-span-2 space-y-6">
            ...
            <div className="flex flex-col sm:flex-row items-center gap-4 bg-slate-900/10 p-3 rounded-2xl border border-white/5 backdrop-blur-md">
              <div className="relative w-full sm:flex-1">
                ...
                <input
                  ...
                  className="w-full rounded-xl border border-white/5 bg-slate-950/40 py-2.5 pl-10 pr-4 text-xs text-white placeholder-slate-500 outline-none transition-all focus:border-cyan-500/40"
                />
              </div>
              ...
              <div className="flex items-center gap-1.5 shrink-0 self-start sm:self-auto">
                ...
                <select
                  ...
                  className="rounded-xl border border-white/5 bg-slate-950/50 py-2 px-3 text-xs text-slate-300 outline-none cursor-pointer focus:border-cyan-500/30"
                >
            ...
            {loading ? (
              <div className="flex flex-col items-center justify-center py-20 space-y-3">
                <Loader2 className="h-8 w-8 animate-spin text-cyan-400" />
                <p className="text-sm text-slate-400">Loading telemetry logs...</p>
              </div>
            ) : filteredLogs.length === 0 ? (
              <div className="rounded-3xl border border-white/5 bg-slate-900/10 p-12 text-center backdrop-blur-sm">
                <h3 className="text-sm font-bold text-white mb-1">No Evaluation Logs</h3>
                  ...
                  {filteredLogs.map((log) => {
                    ...
                    return (
                      <motion.div
                        ...
                        exit={{ opacity: 0 }}
                        className="rounded-2xl border border-white/5 bg-slate-900/30 backdrop-blur-md overflow-hidden transition-all duration-300 hover:border-white/10"
                      >
                        {/* Summary Header bar */}
                        <div
                        ...
                        >
                          <div className="flex items-center gap-3 overflow-hidden min-w-0 flex-1">
                            {/* Route Badge icon */}
                            <div className={`p-2 rounded-xl border shrink-0 ${ROUTE_COLORS[log.route || 'general'] || 'text-slate-400 bg-white/5 border-white/5'}`}>
                            ...
                            {hasEval && log.evaluation && (
                              <div className="hidden sm:flex items-center gap-3 text-[10px] font-bold">
                                <div className="text-center">
                                  <span className="text-slate-500 block text-[9px] font-normal uppercase">Faith</span>
                                  <span className={log.evaluation.faithfulness && log.evaluation.faithfulness >= 0.8 ? 'text-emerald-400' : 'text-rose-400'}>
                                    {Math.round((log.evaluation.faithfulness || 0) * 100)}%
                                  </span>
                                </div>
                                <div className="text-center">
                                  <span className="text-slate-500 block text-[9px] font-normal uppercase">Relev</span>
                                  <span className={log.evaluation.relevance && log.evaluation.relevance >= 0.8 ? 'text-emerald-400' : 'text-rose-400'}>
                        ...
                        {isExpanded && (
                          <motion.div
                            ...
                            className="border-t border-white/5 bg-slate-950/40 p-4 text-xs space-y-4"
                          >
                            {/* Telemetry metadata Grid */}
                            <div className="grid grid-cols-2 sm:grid-cols-4 gap-3.5">
                              <div className="rounded-xl border border-white/5 bg-white/5/20 p-2.5">
                                <span className="text-[10px] text-slate-500 block uppercase font-bold">Estimated Cost</span>
                                <span className="text-emerald-400 font-bold text-sm mt-0.5 block">${log.estimated_cost?.toFixed(5)}</span>
                              </div>
                              <div className="rounded-xl border border-white/5 bg-white/5/20 p-2.5">
                                <span className="text-[10px] text-slate-500 block uppercase font-bold">Total Tokens</span>
                                <span className="text-slate-200 font-bold text-sm mt-0.5 block">{(log.prompt_tokens || 0) + (log.completion_tokens || 0)}</span>
                              </div>
                              <div className="rounded-xl border border-white/5 bg-white/5/20 p-2.5">
                                <span className="text-[10px] text-slate-500 block uppercase font-bold">Prompt Tokens</span>
                                <span className="text-slate-300 font-semibold text-xs mt-0.5 block">{log.prompt_tokens}</span>
                              </div>
                              <div className="rounded-xl border border-white/5 bg-white/5/20 p-2.5">
                                <span className="text-[10px] text-slate-500 block uppercase font-bold">Completion Tokens</span>
                                <span className="text-slate-300 font-semibold text-xs mt-0.5 block">{log.completion_tokens}</span>
                              </div>
                            </div>
                                ...
                                <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                                  ...
                                  <div>
                                    <div className="flex justify-between text-[11px] mb-1 font-semibold">
                                      ...
                                      <span className="text-emerald-400">{Math.round((log.evaluation.faithfulness || 0) * 100)}%</span>
                                    </div>
                                    <div className="h-1.5 w-full rounded-full bg-white/5 overflow-hidden">
                                      <div 
                                  ...
                                  <div>
                                    <div className="flex justify-between text-[11px] mb-1 font-semibold">
                                      ...
                                      <span className="text-cyan-400">{Math.round((log.evaluation.relevance || 0) * 100)}%</span>
                                    </div>
                                    <div className="h-1.5 w-full rounded-full bg-white/5 overflow-hidden">
                                      <div 
                                  ...
                                  <div>
                                    <div className="flex justify-between text-[11px] mb-1 font-semibold">
                                      ...
                                      <span className={log.evaluation.hallucination_risk && log.evaluation.hallucination_risk > 0.2 ? 'text-rose-400' : 'text-slate-400'}>
                                    ...
                                    <div className="h-1.5 w-full rounded-full bg-white/5 overflow-hidden">
                                      <div 
                                        className={`h-full rounded-full ${
                                          log.evaluation.hallucination_risk && log.evaluation.hallucination_risk > 0.2 ? 'bg-rose-500' : 'bg-slate-500'
                                        }`}
