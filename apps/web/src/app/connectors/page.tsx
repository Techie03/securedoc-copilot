  ...
  return (
    <div className="relative min-h-[calc(100vh-4rem)] bg-slate-950 py-8 px-4 sm:px-6 lg:px-8 w-full flex flex-col">
      ...
      <div className="mx-auto max-w-5xl w-full flex-1 flex flex-col">
        ...
        {error && (
          <div className="mt-6 rounded-xl border border-rose-500/20 bg-rose-500/10 p-4 text-sm text-rose-400 flex items-start gap-3">
        ...
        <div className="mt-8">
          <h2 className="text-sm font-bold text-slate-300 uppercase tracking-wider mb-4">Add New Source</h2>
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            
            {/* GitHub Card */}
            <motion.div 
              whileHover={{ y: -2 }}
              className="rounded-2xl border border-white/5 bg-slate-900/40 p-5 backdrop-blur-md flex items-center justify-between group cursor-pointer hover:border-emerald-500/30 transition-all"
              onClick={() => setShowGithubModal(true)}
            >
              <div className="flex items-center gap-4">
                <div className="flex h-12 w-12 items-center justify-center rounded-xl bg-slate-800 text-slate-300 group-hover:text-emerald-400 group-hover:bg-emerald-500/10 transition-colors">
                  <Github className="h-6 w-6" />
                </div>
                <div>
                  <h3 className="font-bold text-white">GitHub Repository</h3>
                  <p className="text-xs text-slate-400 mt-0.5">Ingest code & markdown</p>
                </div>
              </div>
            ...
            <motion.div 
              whileHover={{ y: -2 }}
              className="rounded-2xl border border-white/5 bg-slate-900/40 p-5 backdrop-blur-md flex items-center justify-between group cursor-pointer hover:border-blue-500/30 transition-all"
              onClick={handleCreateGdrive}
            >
              <div className="flex items-center gap-4">
                <div className="flex h-12 w-12 items-center justify-center rounded-xl bg-slate-800 text-slate-300 group-hover:text-blue-400 group-hover:bg-blue-500/10 transition-colors">
                  <HardDrive className="h-6 w-6" />
                </div>
                <div>
                  <h3 className="font-bold text-white flex items-center gap-2">
                    Google Drive
                    <span className="text-[9px] font-bold uppercase bg-blue-500/20 text-blue-400 px-1.5 py-0.5 rounded">Soon</span>
                  </h3>
                  <p className="text-xs text-slate-400 mt-0.5">OAuth placeholder (Phase 6)</p>
                </div>
              </div>
          ...
          <div className="space-y-4">
            {!loading && connectors.length === 0 ? (
              <div className="rounded-2xl border border-dashed border-white/10 p-8 text-center">
                ...
                <h3 className="text-sm font-medium text-slate-300">No active connectors</h3>
                <p className="text-xs text-slate-500 mt-1">Add a source above to automatically ingest data.</p>
              </div>
            ) : (
              connectors.map(conn => (
                <div key={conn.id} className="rounded-2xl border border-white/10 bg-slate-900/50 p-5 backdrop-blur-sm">
                  <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                    
                    {/* Left: Info */}
                    <div className="flex items-center gap-4">
                      <div className={`flex h-12 w-12 items-center justify-center rounded-xl ${
                        conn.provider === 'github' ? 'bg-emerald-500/10 text-emerald-400 border border-emerald-500/20' : 
                      ...
                      <div>
                        <h3 className="font-bold text-white flex items-center gap-2">
                          ...
                          {conn.status === 'syncing' && <span className="flex items-center gap-1 text-[10px] uppercase font-bold text-amber-400 bg-amber-400/10 px-2 py-0.5 rounded-full"><Loader2 className="h-3 w-3 animate-spin"/> Syncing</span>}
                          {conn.status === 'connected' && <span className="flex items-center gap-1 text-[10px] uppercase font-bold text-emerald-400 bg-emerald-400/10 px-2 py-0.5 rounded-full"><CheckCircle2 className="h-3 w-3"/> Connected</span>}
                          {conn.status === 'error' && <span className="flex items-center gap-1 text-[10px] uppercase font-bold text-rose-400 bg-rose-400/10 px-2 py-0.5 rounded-full"><AlertCircle className="h-3 w-3"/> Error</span>}
                          {conn.status === 'pending_oauth' && <span className="flex items-center gap-1 text-[10px] uppercase font-bold text-blue-400 bg-blue-400/10 px-2 py-0.5 rounded-full">Pending Setup</span>}
                        </h3>
                    ...
                    <div className="flex items-center gap-2">
                      {conn.provider === 'github' && (
                        <button
                          ...
                          className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg border border-white/10 text-xs font-medium text-slate-300 hover:bg-white/5 hover:text-white transition-colors disabled:opacity-50"
                        >
                      ...
                      <button
                        ...
                        className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg border border-rose-500/20 text-xs font-medium text-rose-400 hover:bg-rose-500/10 transition-colors"
                      >
                  ...
                  {conn.latest_sync?.logs && (
                    <div className="mt-4 pt-4 border-t border-white/5">
                      <p className="text-[10px] font-bold text-slate-500 uppercase tracking-wider mb-2">Latest Sync Logs</p>
                      <div className="bg-slate-950 rounded-lg p-3 overflow-x-auto">
                        <pre className="text-[10px] text-slate-400 font-mono whitespace-pre-wrap">{conn.latest_sync.logs}</pre>
                      </div>
                    </div>
                  )}
        ...
        {showGithubModal && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
            ...
            <motion.div
              ...
              className="relative w-full max-w-md overflow-hidden rounded-3xl border border-white/10 bg-slate-900 p-6 shadow-2xl backdrop-blur-xl"
            >
              ...
              <form onSubmit={handleCreateGithub} className="space-y-4">
                ...
                <div className="grid grid-cols-2 gap-3">
                  <div>
                    <label className="block text-[10px] font-bold uppercase tracking-wider text-slate-400 mb-1">Owner / Org</label>
                    <input
                      required
                      value={githubOwner}
                      onChange={e => setGithubOwner(e.target.value)}
                      placeholder="e.g. vercel"
                      className="w-full rounded-xl border border-white/10 bg-slate-950/50 p-2.5 text-sm text-white placeholder-slate-500 focus:border-emerald-500 outline-none transition-colors"
                    />
                  </div>
                  <div>
                ...
                <div>
                  <label className="block text-[10px] font-bold uppercase tracking-wider text-slate-400 mb-1">Personal Access Token (Optional)</label>
                  <input
                    type="password"
                    value={githubToken}
                    ...
                    className="w-full rounded-xl border border-white/10 bg-slate-950/50 p-2.5 text-sm text-white placeholder-slate-500 focus:border-emerald-500 outline-none transition-colors"
                  />
                  <p className="text-[10px] text-slate-500 mt-1">Required only for private repositories.</p>
                </div>
